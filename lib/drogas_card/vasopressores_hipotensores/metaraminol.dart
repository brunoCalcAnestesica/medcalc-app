...
Widget _buildCardMetaraminol(
  ...
  return Column(
    ...
    children: [
      ...
      const Text('Metaraminol', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
      ...
      _linhaPreparo('10mg em 500mL SF 0,9%', '20 mcg/mL'),
      _linhaPreparo('10mg em 20mL SF 0,9%', 'Push dose: 500 mcg/mL'),
      ...
      if (isAdulto) ...[
        _linhaIndicacaoDoseCalculada(
          ...
          descricaoDose: '0,5–5 mg IV bolus lento',
        ...
        _linhaIndicacaoDoseCalculada(
          ...
          descricaoDose: '0,5–5 mg/h IV contínua',
          peso: peso,
        ),
      ] else ...[
        _linhaIndicacaoDoseCalculada(
          titulo: 'Hipotensão pediátrica',
          descricaoDose: '10–100 mcg/kg IV bolus',
        ...
        _linhaIndicacaoDoseCalculada(
          titulo: 'Infusão contínua',
          descricaoDose: '1–5 mcg/kg/min IV contínua',
          peso: peso,
        ),
      ],
      ...
      if (faixaEtaria == 'Recém-nascido') ...[
        _textoObs('• Neonatos (0–28 dias): uso restrito à hipotensão refratária em ambiente de UTI neonatal sob monitoração invasiva.'),
      ] else if (faixaEtaria == 'Lactente' || faixaEtaria == 'Criança') ...[
        _textoObs('• Lactentes e crianças pequenas (1 mês a 6 anos): uso em hipotensão associada à anestesia geral ou regional.'),
      ] else if (faixaEtaria == 'Adolescente' || faixaEtaria == 'Adulto') ...[
        _textoObs('• Crianças maiores, adolescentes e adultos: uso amplamente aceito em hipotensão perioperatória e choque distributivo.'),
      ] else if (faixaEtaria == 'Idoso') ...[
        _textoObs('• Idosos: uso com cautela devido ao risco aumentado de isquemia coronariana e cerebral.'),
      ],
      ...
      ConversaoInfusaoSlider(
        peso: 1, // peso fictício para cálculo estático
        opcoesConcentracoes: {
          '10mg/100mL (100mcg/mL)': 100,
          '10mg/250mL (40mcg/mL)': 40,
          '10mg/500mL (20mcg/mL)': 20,
          '500 mcg/mL (500mcg/mL)' : 500,
        },
        unidade: 'mg/h',
        doseMin: 0.5,
        doseMax: 5.0,
      ...
      const SizedBox(height: 8),
      _textoObs('• Agonista alfa-1 adrenérgico de ação prolongada.'),
      _textoObs('• Vasoconstritor potente com efeito sustentado.'),
      _textoObs('• Pode causar isquemia periférica e visceral.'),
      _textoObs('• Monitorar sinais de isquemia e função renal.'),
      _textoObs('• Útil para hipotensão refratária.'),

      const SizedBox(height: 16),
      const Text('Metabolismo', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
      const SizedBox(height: 8),
      _textoObs('• Metabolismo hepático mínimo.'),
      _textoObs('• Excreção predominantemente renal inalterado.'),
      _textoObs('• Duração de ação: 20–60 minutos.'),
...
class MedicamentoMetaraminol {
  ...
  static const String idBulario = 'bulario_metaraminol';

  static Future<Map<String, dynamic>> carregarBulario() async {
    final String jsonStr = await rootBundle.loadString('assets/medicamentos/bulario_metaraminol.json');
